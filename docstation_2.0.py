...
def record(runtime):
        ...
        if True :
                        ...
                        # photos will save to the USB drive named USBDRV (UUID F07D-17E8
                        # we wrote this if statement to fix the problem where sometimes USBDRV is in /media/pi and sometimes it is in /home/pi
                        # if the USB drive has not been used on the pi before, will create folder named "Doc_Station_Photos" to store the pictures
                        # we opted to use the makedirs function instead of 'touch' because touch relies on timestamps, and we are not confident in the reliability of our pi's timestamps (since time and date need to be manually set right now)
			if os.path.exists("/media/pi/USBDRV/"):
				if not os.path.exists("/media/pi/USBDRV/Doc_Station_Photos/"):
					os.makedirs("/media/pi/USBDRV/Doc_Station_Photos/")
				usbname= "/media/pi/USBDRV/Doc_Station_Photos/%(1)s_%(2)s_%(3)s.jpg" % {"1": hi,"2": ti,"3" : i}
			elif os.path.exists("/home/pi/USBDRV/"):
				if not os.path.exists("/home/pi/USBDRV/Doc_Station_Photos/"):
					os.makedirs("/home/pi/USBDRV/Doc_Station_Photos/")
				usbname= "/home/pi/USBDRV/Doc_Station_Photos/%(1)s_%(2)s_%(3)s.jpg" % {"1": hi, "2": ti, "3": i}
			else: 
				# if there is no USB, the files will just save locally
				usbname= "/home/pi/Photos/%(1)s_%(2)s_%(3)s.jpg" % {"1": hi,"2": ti,"3" : i}
                       
                        #consider having a second upload that doesnt name by date so that the slideshow can just run thru it usbname = "/home/pi/Photos/_%i.jpg" %i  
